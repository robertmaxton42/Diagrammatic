...
type NodeControl() as this =
  ...
  let rotateVector (v: Vector) θ =
    Vector(v.X * cos θ + v.Y * sin θ, v.Y * cos θ - v.X * sin θ)

  let getPortPoint (portData: PortViewData) =
    let getLinePoint t (s: Point, e: Point) = s * (1.0 - t) + e * t

    let getPolyPoint n t (poly: PolylineGeometry) =
      getLinePoint t (poly.Points.Item n, poly.Points.Item(n + 1))
    ...
    | :? EllipseGeometry as ellipse ->
      ellipse.Center
      + Vector(ellipse.RadiusX * (sin portData.t), ellipse.RadiusY * (cos portData.t))
  ...
  member this.Node = this.DataContext :?> LabeledNodeWrapper

  member this.Ports: LabeledPort<PortViewData, EdgeViewData> seq =
    seq {
  ...
  member private this.RenderPorts() =
    this.RenderedPorts <-
      AvaloniaList(Seq.map (fun (port: LabeledPort<_, _>) -> getPortPoint (snd port).Value) this.Ports)
