...
(*  
  ...
  Equality specifically will be handled by a Derivation class, that batches the successive steps
   of a derivation
*)
type EdgeKind = SimpleEdge of string
[<CustomEquality>]
[<CustomComparison>]
type Node<'p, 'e> = 
  struct
...
and DiagramEdge<'label> = 
  struct
    val kind: EdgeKind
    val label: 'label ref
    new (kind, label) = { kind = kind; label = label }
  end
//and LabeledEdge<'p, 'e> = LEdge<Port<'p, 'e>, Edge<'e>>
...
type NodeTemplate<'p, 'e>(kind: NodeKind<'p, 'e>, port_structure: PortGroup<'p, 'e> seq) =
